...
const createSupabaseClientWithStorage = (persistSession = true) => {
    ...
    return createClient(supabaseUrl, supabaseAnonKey, {
        auth: {
            ...
            persistSession: persistSession && isBrowser
        }
    });
};
...
/**
 * Generate embeddings locally through the server-side Ollama endpoint.
 * The browser never downloads an embedding model and no zero-padding is used.
 */
export const generateEmbeddings = async (texts) => {
    ...
    for (let index = 0; index < input.length; index += 50) {
        ...
        const response = await fetch(`${config.API_URL}/embeddings`, {
            method: 'POST',
        ...
        if (!response.ok) {
            throw new Error(data.error || `Embedding gagal dengan status ${response.status}`);
        }
...
export const uploadDocument = async (file, userId, sections, isShared = false) => {
    try {
        ...
        const sectionsData = sections.map((section, chunkIndex) => ({
            document_id: docData.id,
            content: section.content,
            chunk_index: chunkIndex,
            embedding_model: 'embeddinggemma',
            embedding_v2: section.embedding
        }));
 ...
 */
let hasWarnedAboutDocumentSearch = false;

const warnDocumentSearchUnavailable = (error) => {
    if (hasWarnedAboutDocumentSearch) return;

    hasWarnedAboutDocumentSearch = true;
    const details = error?.message || error?.code || 'Supabase document search is unavailable';
    console.warn(`Document search dilewati: ${details}`);
};
...
export const searchPersonalDocuments = async (query, threshold = 0.1, count = 5) => {
    try {
        ...
        if (error) {
            // Document search is optional. A missing RPC must not stop local chat
            // or trigger the Next.js development error overlay.
            warnDocumentSearchUnavailable(error);
            return [];
        }
