...
import { Menu, Shield, PanelRightOpen, PanelRightClose, Sparkles, BookOpen, Presentation, FileText, Share2, Database, Headset, CheckCircle2, Circle, Loader2 } from 'lucide-react';
...
const THINKING_STAGES = [
  { id: 'preparing', label: 'Menyiapkan percakapan' },
  { id: 'retrieving', label: 'Memeriksa dokumen dan base knowledge' },
  { id: 'generating', label: 'Menghubungi model AI' },
...
const ChatPage = () => {
  ...
  const [thinkingStage, setThinkingStage] = React.useState('preparing');
  ...
  const [aiNewsOpen, setAiNewsOpen] = React.useState(false);
  ...
  - [Point 2]
        ...
        if (!response.ok) {
          throw new Error(data.response || `Permintaan AI gagal dengan status ${response.status}`);
        }
  ...
  const handleResendUserMessage = async (messageId) => {
    ...
    if (messageIndex === -1 || activeConversation.messages[messageIndex].role !== 'user') return;

    const messages = activeConversation.messages.slice(0, messageIndex + 1);
    const userMessage = { ...messages[messageIndex], ragContext: undefined };
    messages[messageIndex] = userMessage;
  ...
  const handleEditUserMessage = async (messageId, content) => {
    ...
    saveConversationToBackend(updatedConv);
    await getAIResponse(updatedConv, editedMessage);
  };
  ...
  const handleFeatureClick = (feature) => {
    ...
    if (config) {
      ...
      } else if (config.action === 'openAINews') {
        setAiNewsOpen(true);
            ...
            <div className="pb-4">
              {activeConversation.messages.map((message, index) => (
                <ChatMessage
                  ...
                  modelName={aiModels.find(model => model.id === message.model)?.name}
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 text-gray-400">
                        ...
                        <span className="text-sm">
                          {THINKING_STAGES.find(stage => stage.id === thinkingStage)?.label || 'Memproses'}
                          <span className="ml-1 text-gray-500">• {thinkingElapsed} dtk</span>
                        </span>
                      </div>
                      <div className="mt-3 max-w-md rounded-xl border border-[#2f2f2f] bg-[#212121]/70 p-3">
                        {THINKING_STAGES.map((stage, stageIndex) => {
                          const currentIndex = THINKING_STAGES.findIndex(item => item.id === thinkingStage);
                          ...
                          return (
                            <div key={stage.id} className="flex items-center gap-2 py-1 text-xs">
                              ...
                              <span className={isCurrent ? 'text-emerald-300' : isComplete ? 'text-gray-300' : 'text-gray-600'}>
                                {stage.label}
                              </span>
                            </div>
                          );
